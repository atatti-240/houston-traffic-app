...
- **Watched plans** (`watch: true`) are re-planned every 5 min until you leave, right away when live data changes, and on the tick your departure comes due, so "leave now" always uses fresh conditions. Their alerts are `plan`, `order_changed`, `leave_earlier`, `leave_later`, `leave_now` (one per leg) and `info`.
  ...
  - A re-plan keeps the current stop order unless another order is better on lateness or saves at least 3 min. That way near-equal orders don't flip back and forth with a "New stop order" alert every few minutes. First departures are tried on quarter-hour clock marks for the same reason.
  - If you never left, every stop's window has closed, and the planned departure has passed too, the plan stops being watched and sends one "Missed" alert (`info`). A plan that's already late still gets its "leave now" at its planned departure. Stops without an end time never expire this way. Once you've left, the remaining legs' "leave now" alerts still come.
