...
- **Later legs** leave when you're ready (arrival + dwell). They leave later only in two cases: to avoid arriving before a stop's window opens (and never so late that you'd get there after it opened), or to avoid sitting at a closed road when leaving later gets you there just as soon. In the closed-road case, "just as soon" means within a minute. It means no later at all if a stop after this one has a deadline, and never late or tight for this stop's own window. For a stop with only an end time, arriving early costs nothing, and waiting at the previous stop would only put the stops after it at risk.
